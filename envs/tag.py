...
class TagEnv(Env):
    ...
    def __init__(self, move_prob=.8, obs_cells=29, board_size=(10, 5)):
        self.move_prob = move_prob
        self._reward_range = [-np.inf, 10]
        self._discount = .95
        self.action_space = Discrete(len(Action))
        self.grid = TagGrid(board_size, obs_cells=obs_cells)
        self.observation_space = Discrete(self.grid.n_tiles + 1)
        self.state_space = Discrete(self.grid.n_tiles*(self.grid.n_tiles+1))
        self.belief = np.zeros(len(self.state_space))
    ...
    def reset(self):
        ...
        index = np.delete(np.arange(index*self.grid.n_tiles,(index+1)*self.grid.n_tiles),index)
        self.belief[index] = 1/28
    ...
    def _decode_state(self, state):
        ...
        opp_pos = self.grid.get_tag_coord(state[1])
        tag_state.opponent_pos= opp_pos
    ...
    def move_opponent(self):
        actions = self._admissable_actions(self.state.agent_pos, self.state.opponent_pos)
        if np.random.binomial(1, self.move_prob):
            move = np.random.choice(actions).value
            if self.grid.is_inside(self.state.opponent_pos + move):
                self.state.opponent_pos = self.state.opponent_pos + move

    def simulate_action(self, action, belief):
        # For model-based approach
        assert self.action_space.contains(action)  # action are idx of movements
        assert self.done is False
        assert self.grid.is_inside(belief)

        self.state = self._sample_state(belief)
        self.last_state = self.state
        ob, _, _, _ = self.step(action)

        return self._update_belief(belief, ob)

    def _update_belief(self, belief, ob):
    ...
    def _compute_prob(self, action, next_state, ob):
        ...
        if ob == self.grid.n_tiles:
            if next_state.opponent_pos == next_state.agent_pos:
                return 1.
        return p_ob

    def _sample_state(self, belief):
        state = TagState(belief)
        for opp in range(self.num_opponents):
            opp_pos = self.grid.sample()
            assert self.grid.is_inside(opp_pos)
            tag_state.opponent_pos[opp] = opp_pos
        return tag_state if not should_encode else self._encode_state(tag_state)
